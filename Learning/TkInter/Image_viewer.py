...
def backward(n):
    ...
    try:
        ...
        label.grid(row=0,column=0,columnspan=3);
        forwardButton=tkinter.Button(window,text=">>",command=lambda : forward(n-1));
        backwardButton=tkinter.Button(window,text="<<",command=lambda : backward(n+1));
        ...
        label.grid(row=0,column=0,columnspan=3);
        backwardButton=tkinter.Button(window,text="<<",command=lambda : backward(0));
        forwardButton=tkinter.Button(window,text=">>",command=lambda : forward(0));
...
def forward(n):
    ...
    try:
        ...
        label=tkinter.Label(image=imagesList[0]);
        label.grid(row=0,column=0,columnspan=3);
        backwardButton=tkinter.Button(window,text="<<",command=backward);
        forwardButton=tkinter.Button(window,text=">>",command=lambda : forward(0));

        backwardButton.grid(row=1,column=0);
        forwardButton.grid(row=1,column=2);

backwardButton=tkinter.Button(window,text="<<",command=lambda : backward(1));
forwardButton=tkinter.Button(window,text=">>",command=lambda : forward(1));
